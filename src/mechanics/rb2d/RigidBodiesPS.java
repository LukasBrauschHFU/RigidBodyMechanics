...
List<RigidBody> rigidBodies = new ArrayList<RigidBody>();
		...
		rigidBodies.add(new RigidBody(Double.MAX_VALUE, new Vector2D(0, 0), new Vector2D(0, 0), new Vector2D(0, 0), Double.MAX_VALUE, Math.PI/8, 5, 0, new Rectangle(2,1)));
	...
	private void test8() {
		...
		rigidBodies.add(new RigidBody(2.7, new Vector2D(-2.3092533854673296, 2.317296134400402), new Vector2D(-0.40556197276118344, -0.7548117969613616), new Vector2D(0, 0), 10, -6.568889443058639, -0.5371540750645915, 0,
				new Polygon(new Vector2D[] {new Vector2D(-1.5, 1.5), new Vector2D(1.5, 1.5), new Vector2D(1.5, -1.5), new Vector2D(-1.5, -1.5),
						new Vector2D(-1, -1), new Vector2D(1, -1), new Vector2D(1, 1), new Vector2D(-1, 1)})));
		rigidBodies.add(new RigidBody(0.7, new Vector2D(1.048279777283462, 2.8125373703172523), new Vector2D(-3.484229851346371, -0.2719497647193772), new Vector2D(0, 0), 0.3, 125.84038262508483, 1, 0,
				new Circle(0.5)));
