...
////		}
//		if (ballState == BallState.ROLLING) {
//			if (checkPointInsideCircle(il.start, r)) {
//				Vector2D vectorToStart = VectorMath.sub(il.start, r);
//				Vector2D tangente = VectorMath.perpendicular(il.start, vectorToStart);
//				System.out.println("tangenete " + tangente);
//				Vector2D point = new Vector2D(r.x, r.y);
//				Runnable handler = new ballRollsOnInclineHandler(this, il, new Impactpoint(point, tangente));
//				aed.reportEvent(handler, "collision ball and line: ", "ball " + this.toString());
//			}
//		}
...
//		}
		...
		if (checkPointInsideCircle(il.start, r) && r.x <= il.start.x-0.1) {
			...
			Vector2D point = new Vector2D(r.x, r.y);
			System.out.println("tangente "+tangente.direction);
